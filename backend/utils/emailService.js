...
exports.sendActivationEmail = async (email, activationUrl) => {
  if (!user || !pass) {
    console.warn('\n⚠️ [EMAIL SERVICE WARNING] SMTP Credentials (SMTP_USER/SMTP_PASS) are not configured in .env. Activation Link logged to console:');
  ...
  const mailOptions = {
    from: `"Harran Üniversitesi Sınav Yönetimi" <${user}>`,
    to: email,
    subject: 'Harran Üniversitesi - Hesap Aktivasyon Talebi',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1a202c;">
        <div style="text-align: center; margin-bottom: 20px; padding: 20px; background-color: #002147; border-radius: 8px;">
          <h2 style="color: #ffffff; margin: 0; font-size: 20px; font-weight: bold; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">Harran Üniversitesi</h2>
        ...
        <p style="font-size: 14px; line-height: 1.6;">Sisteme giriş yapabilmeniz için öncelikle e-posta adresinizi doğrulamanız gerekmektedir. Aşağıdaki butona tıklayarak hesabınızı aktif hale getirebilirsiniz:</p>
        
        <div style="text-align: center; margin: 30px 0;">
          <a href="${activationUrl}" style="background-color: #00306e; color: #ffffff; padding: 12px 28px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px; display: inline-block; box-shadow: 0 4px 6px rgba(0,48,110,0.15);">Hesabımı Aktifleştir</a>
        </div>
        
        <p style="font-size: 12px; color: #718096; margin-top: 40px; border-top: 1px solid #e2e8f0; padding-top: 20px; line-height: 1.6;">
          Eğer yukarıdaki butona tıklayamıyorsanız, aşağıdaki bağlantıyı tarayıcınızın adres çubuğuna kopyalayıp yapıştırabilirsiniz:<br>
          <a href="${activationUrl}" style="color: #00306e; text-decoration: underline; word-break: break-all;">${activationUrl}</a>
        </p>
